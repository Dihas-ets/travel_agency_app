...
class PercepteurColisContent extends StatefulWidget {
  const PercepteurColisContent({super.key});

  @override
  State<PercepteurColisContent> createState() => PercepteurColisContentState();
}

class PercepteurColisContentState extends State<PercepteurColisContent> {
  ...
  List<PercepteurParcelRecord> get _filteredParcels {
    ...
    return _availableParcels.where((parcel) {
      return parcel.id.toLowerCase().contains(query) ||
          ...
          parcel.percepteurPhone.toLowerCase().contains(query);
    }).toList();
  }

  PercepteurParcelRecord? get _selectedParcel {
    if (_selectedPhone == null) return null;
    for (final parcel in _availableParcels) {
      if (parcel.percepteurPhone == _selectedPhone) return parcel;
    }
    return null;
  }
  ...
  void _removeParcel(PercepteurParcelRecord parcel) {
    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
  ...
  void _toggleStatus(PercepteurParcelRecord parcel) {
    setState(() {
      parcel.status = parcel.status == 'Arriver' ? 'Recuperer' : 'Arriver';
    });
  }

  void _showParcelDetail(PercepteurParcelRecord parcel) {
    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        ...
        content: Column(
          ...
          children: [
            ...
            Text('Percepteur : ${parcel.percepteurPhone}'),
  ...
  Widget _buildEmbarquementContent(PercepteurParcelRecord? selectedParcel) {
    return Stack(
      children: [
        Positioned.fill(
          child: ListView(
                    ...
                    LayoutBuilder(
                      builder: (context, constraints) {
                        ...
                        final phoneField = DropdownButtonFormField<String>(
                          ...
                          isExpanded: true,
                          decoration: percepteurColisInputDecoration(
                          ...
                          items: _filteredParcels
                              .map(
                                (parcel) => DropdownMenuItem(
                                  value: parcel.percepteurPhone,
                                  child: Text(parcel.percepteurPhone),
                                ),
                              )
                              .toList(),
                          onChanged: (value) =>
...
class PercepteurColisModeTabs extends StatelessWidget {
  final int selectedIndex;
  final ValueChanged<int> onChanged;

  const PercepteurColisModeTabs({
    super.key,
...
class PercepteurColisModeButton extends StatelessWidget {
  final String label;
  final IconData icon;
  final bool selected;
  final VoidCallback onTap;

  const PercepteurColisModeButton({
    super.key,
    required this.label,
    required this.icon,
...
InputDecoration percepteurColisInputDecoration({
...
class PercepteurTransitParcelTable extends StatelessWidget {
  final List<PercepteurParcelRecord> parcels;
  final ValueChanged<PercepteurParcelRecord> onView;
  final ValueChanged<PercepteurParcelRecord> onRemove;
  final ValueChanged<PercepteurParcelRecord> onToggleStatus;

  const PercepteurTransitParcelTable({
    super.key,
